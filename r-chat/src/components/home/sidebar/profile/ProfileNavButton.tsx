...
function ProfileNavButton({ setError, setErrorMessage }: { setError: Dispatch<boolean>, setErrorMessage: Dispatch<string> }) {
    ...
    return (
        <>
            <div data-testid='profile-nav-button' onClick={() => setProfileOpened(!profileOpened)} className='w-full text-white min-h-[3rem] cursor-pointer flex items-center transition-all ease-in-out 150 hover:text-primary'>
